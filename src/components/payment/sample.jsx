...
const InvoiceCreate = () => {
  ...
  const fetchProductsData = async () => {
    try {
      ...
      const response = await fetch(
        "https://localhost:7053/api/Product/get-productlist?Organization=1e7071f0-dacb-4a98-f264-08dcb066d923",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const data = await response.json();
      // Transform data to match AutoComplete's expected format
  ...
  useEffect(() => {
    const newTotals = calculateTotals(rows, discountInput);
    setTotals(newTotals);
  }, [rows, discountInput]);
  ...
  const handleApplyDiscount = () => {
    // Recalculate totals when the "Apply" button is clicked
    const newTotals = calculateTotals(rows, discountInput);
    setTotals(newTotals);
  }; 
